...
import Natural.Unary as Natural exposing (Natural)
...
naturalFuzzer =
  -- Why an Fuzz.intRange and not Fuzz.int?
  --
  -- The integers generated could be so large that
  -- it causes a "JavaScript heap out of memory" error.
  --
  -- Remember that the unary representation uses lists.
